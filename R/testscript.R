...
ggplot() +
  ...
  geom_point(aes(x = occ_matched$decimalLongitude, y = occ_matched$decimalLatitude, color = factor(occ_matched$species)), size=0.001)
...
ggplot(data = occ_matched, aes(x = decimalLongitude, y = decimalLatitude, color = factor(species))) +
  ...
  labs(color = "Species")
########################
ggplot(data = occ_matched, aes(x = decimalLongitude, y = decimalLatitude)) +
  geom_map(data = norway, map = norway, aes(long, lat, map_id=region), color="#2b2b2b", fill="white") +
  geom_point(size=0.001, color = "red3") +
  facet_wrap(~species)
