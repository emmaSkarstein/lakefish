...
ggplot(occ) +
  geom_map(data = norway, map = norway, aes(long, lat, map_id=region), 
           color="#2b2b2b", fill = "white") + 
  geom_point(aes(x = decimalLongitude, y = decimalLatitude, color = species), alpha = 0.6, size = 0.1) + 
  scale_color_brewer(palette="Paired") +
  theme(axis.title.x = element_blank(), 
        axis.title.y = element_blank()) +
  guides(colour = guide_legend(override.aes = list(size=2))) + 
...
```{r species_richness}
rich_lakes <- occ %>% group_by(waterBody) %>% summarize(unique_spec = n_distinct(species))
colnames(rich_lakes) <- c("waterBody", "unique_spec", "geometry")
rich_lakes <- st_set_geometry(rich_lakes, NULL)
rich_lakes_w_loc <- merge(rich_lakes, occ, by = "waterBody")

ggplot(rich_lakes_w_loc) + 
  geom_map(data = norway, map = norway, aes(long, lat, map_id=region), 
           color="#2b2b2b", fill = "white") + 
  geom_point(aes(x = decimalLongitude, y = decimalLatitude, color = unique_spec), alpha = 0.5, size = 1) +
...
```{r, eval = TRUE}
head(arrange(rich_lakes, by_group = unique_spec))
```
